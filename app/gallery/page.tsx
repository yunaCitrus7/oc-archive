...
export default function GalleryPage(){
 ...
 return <main className="notebook-page gallery-page">
  <nav className="nav"><a className="brand" href="/"><span>CITRUSODA</span> OC ARCHIVE</a><div className="navlinks"><a href="/">CHARACTERS</a><a href="/comics">COMICS</a></div></nav>
  <section className="gallery-shell"><a className="back" href="/">← BACK TO ARCHIVE</a><h1>Gallery</h1>
   ...
   <div className="gallery-layout">
    <aside className={`gallery-filters${filterOpen?" open":""}`}><h2>Filter</h2>
     ...
     <div className="filter-group"><b>Character</b><div className="character-filter">{characters.map(x=><button className={character===x?"active":""} onClick={()=>setCharacter(character===x?"All":x)} key={x} aria-label={x}><img src={`/icons/${x}.svg`} alt=""/></button>)}</div></div>
    </aside>
    <section key={`${category}-${year}-${character}`} className="gallery-package"><div className="gallery-handle" aria-hidden="true"/><div className="gallery-art-grid">{shown.map((x,index)=><figure key={x.character}><button className="gallery-art-button" type="button" onClick={()=>setLightbox(index)}><img src={`/characters/${x.character}.png`} alt={`${x.character} artwork`}/></button><figcaption><b>{x.character.toUpperCase()}</b><span>{x.category} · {x.year}</span></figcaption></figure>)}{shown.length===0&&<p className="gallery-empty">No artwork matches these filters yet.</p>}</div></section>
   </div>
  </section>
  {current&&<div className="gallery-lightbox" role="dialog" aria-modal="true" aria-label={`${current.character} artwork`} onClick={()=>setLightbox(null)}><button className="lightbox-close" onClick={()=>setLightbox(null)} aria-label="Close">×</button><button className="lightbox-arrow prev" onClick={e=>{e.stopPropagation();move(-1)}} aria-label="Previous image">‹</button><div className="lightbox-image" onClick={e=>e.stopPropagation()}><img src={`/characters/${current.character}.png`} alt={`${current.character} artwork`}/><p>{current.character.toUpperCase()} · {current.category} · {current.year}</p></div><button className="lightbox-arrow next" onClick={e=>{e.stopPropagation();move(1)}} aria-label="Next image">›</button></div>}
 </main>
}

...
export default function MobileNav(){
 ...
 return <div className={`mobile-site-menu${open?" open":""}`}>
  ...
  <nav aria-label="Mobile navigation"><a href={sitePath("/#characters")}>{language==="en"?"CHARACTERS":"角色介紹"}</a><a href={sitePath("/comics")}>{language==="en"?"COMICS":"漫畫合集"}</a><a href={sitePath("/gallery")}>{language==="en"?"GALLERY":"繪畫合集"}</a><a href={sitePath("/relationships")}>{language==="en"?"RELATIONSHIPS":"人物關係"}</a></nav>
 </div>
}
